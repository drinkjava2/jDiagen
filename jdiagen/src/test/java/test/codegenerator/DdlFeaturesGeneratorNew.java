	...
	public void exportOtherFeatures() { 
		...
		for (Class<? extends Dialect> hibDialectClass : dialects) {
			...
			dao.nExecute("alter table tb_hibdll add  " + d.getClass().getSimpleName() + " varchar(500)");
		       ...
		       try{dealOneFeature(d,"addFKeyRefPkeyString", ""+d.getAddForeignKeyConstraintString("_FKEYNAME", _FKS, "_REFTABLE", _REFS, true));}catch(Exception e){dealOneFeature(d,"addForeignKeyConstraintString", NOT_SUPPORT);}                                                                                                                
