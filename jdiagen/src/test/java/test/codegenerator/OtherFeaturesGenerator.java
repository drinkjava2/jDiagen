...
/**
 * This is not a unit test class, it's a code generator tool to collect extract
 * data from Hibernate to Dialect
 *
 * @author Yong Zhu
 *
 * @version 1.0.0
 * @since 1.0.0
 */
public class OtherFeaturesGenerator {
	...
	@Test
	public void collectOtherFeatures() {
	...
	public void dealOneFeature(Dialect d, String feature, String featureValue) {
		...
		Dao.execute("update tb_Others set "//
				, d.getClass().getSimpleName(), "=", q(featureValue), " where feature=", q(feature));
	}
...
//		    try{dealOneFeature(d,"getDialect", ""+d.getDialect(Properties));}catch(Exception e){dealOneFeature(d,"getDialect(Properties)", e.getMessage());}
...
//			try{dealOneFeature(d,"getHibernateTypeName", ""+d.getHibernateTypeName(int, int, int, int));}catch(Exception e){dealOneFeature(d,"getHibernateTypeName(int, int, int, int)", e.getMessage());}
			...
			try{dealOneFeature(d,"supportsSequences", ""+d.supportsSequences());}catch(Exception e){dealOneFeature(d,"supportsSequences", e.getMessage());}
			try{dealOneFeature(d,"supportsPooledSequences", ""+d.supportsPooledSequences());}catch(Exception e){dealOneFeature(d,"supportsPooledSequences", e.getMessage());}
//			try{dealOneFeature(d,"getSequenceNextValString", ""+d.getSequenceNextValString(String));}catch(Exception e){dealOneFeature(d,"getSequenceNextValString(String)", e.getMessage());}
//			try{dealOneFeature(d,"getSelectSequenceNextValString", ""+d.getSelectSequenceNextValString(String));}catch(Exception e){dealOneFeature(d,"getSelectSequenceNextValString(String)", e.getMessage());}
//			try{dealOneFeature(d,"getCreateSequenceStrings", ""+d.getCreateSequenceStrings(String));}catch(Exception e){dealOneFeature(d,"getCreateSequenceStrings(String)", e.getMessage());}
//			try{dealOneFeature(d,"getCreateSequenceStrings", ""+d.getCreateSequenceStrings(String, int, int));}catch(Exception e){dealOneFeature(d,"getCreateSequenceStrings(String, int, int)", e.getMessage());}
//			try{dealOneFeature(d,"getDropSequenceStrings", ""+d.getDropSequenceStrings(String));}catch(Exception e){dealOneFeature(d,"getDropSequenceStrings(String)", e.getMessage());}
...
//			try{dealOneFeature(d,"getLimitString", ""+d.getLimitString(String, int, int));}catch(Exception e){dealOneFeature(d,"getLimitString(String, int, int)", e.getMessage());}
//			try{dealOneFeature(d,"convertToFirstRowValue", ""+d.convertToFirstRowValue(int));}catch(Exception e){dealOneFeature(d,"convertToFirstRowValue(int)", e.getMessage());}
...
//			try{dealOneFeature(d,"getForUpdateString", ""+d.getForUpdateString(String));}catch(Exception e){dealOneFeature(d,"getForUpdateString(String)", e.getMessage());}
//			try{dealOneFeature(d,"getForUpdateString", ""+d.getForUpdateString(String, LockOptions));}catch(Exception e){dealOneFeature(d,"getForUpdateString(String, LockOptions)", e.getMessage());}
...
//			try{dealOneFeature(d,"applyLocksToSql", ""+d.applyLocksToSql(String, LockOptions, Map<String, String[]>));}catch(Exception e){dealOneFeature(d,"applyLocksToSql(String, LockOptions, Map<String, String[]>)", e.getMessage());}
			try{dealOneFeature(d,"getCreateTableString", ""+d.getCreateTableString());}catch(Exception e){dealOneFeature(d,"getCreateTableString", e.getMessage());}
			try{dealOneFeature(d,"getCreateMultisetTableString", ""+d.getCreateMultisetTableString());}catch(Exception e){dealOneFeature(d,"getCreateMultisetTableString", e.getMessage());}
...
//			try{dealOneFeature(d,"quote(String)", ""+d.quote(String));}catch(Exception e){dealOneFeature(d,"quote(String)", e.getMessage());}
...
//			try{dealOneFeature(d,"getCreateCatalogCommand(String)", ""+d.getCreateCatalogCommand(String));}catch(Exception e){dealOneFeature(d,"getCreateCatalogCommand(String)", e.getMessage());}
//			try{dealOneFeature(d,"getDropCatalogCommand(String)", ""+d.getDropCatalogCommand(String));}catch(Exception e){dealOneFeature(d,"getDropCatalogCommand(String)", e.getMessage());}
			try{dealOneFeature(d,"canCreateSchema", ""+d.canCreateSchema());}catch(Exception e){dealOneFeature(d,"canCreateSchema", e.getMessage());}
//			try{dealOneFeature(d,"getCreateSchemaCommand(String)", ""+d.getCreateSchemaCommand(String));}catch(Exception e){dealOneFeature(d,"getCreateSchemaCommand(String)", e.getMessage());}
//			try{dealOneFeature(d,"getDropSchemaCommand(String)", ""+d.getDropSchemaCommand(String));}catch(Exception e){dealOneFeature(d,"getDropSchemaCommand(String)", e.getMessage());}
			try{dealOneFeature(d,"getCurrentSchemaCommand", ""+d.getCurrentSchemaCommand());}catch(Exception e){dealOneFeature(d,"getCurrentSchemaCommand", e.getMessage());}
			...
			try{dealOneFeature(d,"hasAlterTable", ""+d.hasAlterTable());}catch(Exception e){dealOneFeature(d,"hasAlterTable", e.getMessage());}
			try{dealOneFeature(d,"dropConstraints", ""+d.dropConstraints());}catch(Exception e){dealOneFeature(d,"dropConstraints", e.getMessage());}
			...
			try{dealOneFeature(d,"getAddColumnString", ""+d.getAddColumnString());}catch(Exception e){dealOneFeature(d,"getAddColumnString", e.getMessage());}
			try{dealOneFeature(d,"getAddColumnSuffixString", ""+d.getAddColumnSuffixString());}catch(Exception e){dealOneFeature(d,"getAddColumnSuffixString", e.getMessage());}
			try{dealOneFeature(d,"getDropForeignKeyString", ""+d.getDropForeignKeyString());}catch(Exception e){dealOneFeature(d,"getDropForeignKeyString", e.getMessage());}
...
//			try{dealOneFeature(d,"getAddForeignKeyConstraintString(String, String[], String, String[], boolean)", ""+d.getAddForeignKeyConstraintString(String, String[], String, String[], boolean));}catch(Exception e){dealOneFeature(d,"getAddForeignKeyConstraintString(String, String[], String, String[], boolean)", e.getMessage());}
//			try{dealOneFeature(d,"getAddForeignKeyConstraintString(String, String)", ""+d.getAddForeignKeyConstraintString(String, String));}catch(Exception e){dealOneFeature(d,"getAddForeignKeyConstraintString(String, String)", e.getMessage());}
//			try{dealOneFeature(d,"getAddPrimaryKeyConstraintString(String)", ""+d.getAddPrimaryKeyConstraintString(String));}catch(Exception e){dealOneFeature(d,"getAddPrimaryKeyConstraintString(String)", e.getMessage());}
			...
			try{dealOneFeature(d,"supportsCommentOn", ""+d.supportsCommentOn());}catch(Exception e){dealOneFeature(d,"supportsCommentOn", e.getMessage());}
//			try{dealOneFeature(d,"getTableComment(String)", ""+d.getTableComment(String));}catch(Exception e){dealOneFeature(d,"getTableComment(String)", e.getMessage());}
//			try{dealOneFeature(d,"getColumnComment(String)", ""+d.getColumnComment(String));}catch(Exception e){dealOneFeature(d,"getColumnComment(String)", e.getMessage());}
			try{dealOneFeature(d,"supportsIfExistsBeforeTableName", ""+d.supportsIfExistsBeforeTableName());}catch(Exception e){dealOneFeature(d,"supportsIfExistsBeforeTableName", e.getMessage());}
			try{dealOneFeature(d,"supportsIfExistsAfterTableName", ""+d.supportsIfExistsAfterTableName());}catch(Exception e){dealOneFeature(d,"supportsIfExistsAfterTableName", e.getMessage());}
			try{dealOneFeature(d,"supportsIfExistsBeforeConstraintName", ""+d.supportsIfExistsBeforeConstraintName());}catch(Exception e){dealOneFeature(d,"supportsIfExistsBeforeConstraintName", e.getMessage());}
			try{dealOneFeature(d,"supportsIfExistsAfterConstraintName", ""+d.supportsIfExistsAfterConstraintName());}catch(Exception e){dealOneFeature(d,"supportsIfExistsAfterConstraintName", e.getMessage());}
//			try{dealOneFeature(d,"getDropTableString(String)", ""+d.getDropTableString(String));}catch(Exception e){dealOneFeature(d,"getDropTableString(String)", e.getMessage());}
...
//			try{dealOneFeature(d,"getAddUniqueConstraintString(String)", ""+d.getAddUniqueConstraintString(String));}catch(Exception e){dealOneFeature(d,"getAddUniqueConstraintString(String)", e.getMessage());}
